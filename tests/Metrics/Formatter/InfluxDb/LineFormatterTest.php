...
namespace Hitmeister\Component\Metrics\Tests\Formatter\InfluxDb;
...
class LineFormatterTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testMetricQuotation()
    {
        ...
        $expected1 = '0metric_name1 value=10i';
        $this->assertEquals($expected1, $this->formatter->format($metric1));

        $metric2 = new CounterMetric('metric_name2', 20, ['method/name' => 'Run/Me']);
        $expected2 = 'metric_name2,method/name=Run/Me value=20i';
        $this->assertEquals($expected2, $this->formatter->format($metric2));

        $metric3 = new CounterMetric('metric_name3', 30, ['tag "with quotes" allowed' => 'tag_value']);
        $expected3 = 'metric_name3,tag\ "with\ quotes"\ allowed=tag_value value=30i';
        $this->assertEquals($expected3, $this->formatter->format($metric3));
     ...
     * Tests sampling
     */
    public function testMetricBoolean()
    {
        $metric1 = new CounterMetric('metric_name1', true);
        $expected1 = 'metric_name1 value=true';
        $this->assertEquals($expected1, $this->formatter->format($metric1));

        $metric2 = new CounterMetric('metric_name2', false);
        $expected2 = 'metric_name2 value=false';
        $this->assertEquals($expected2, $this->formatter->format($metric2));
    }
}